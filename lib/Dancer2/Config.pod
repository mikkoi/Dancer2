...
=item * C<build_config_files> is called after the payload has been serialized, and
receives the serialized content as an argument.

  hook build_config_files => sub {
    my $serialized_content = shift;
    ...
  };

=item * C<build_config> is called before serializing the content, and receives
the content to serialize as an argument.

  hook build_config => sub {
    my $content = shift;
    ...
  };
